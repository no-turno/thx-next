<div style="display: flex; align-items: center; gap: 16px; padding: 8px; border-radius: 6px; width: fit-content; background: rgb(3,0,52);
background: rgb(3,0,52);
background: -moz-linear-gradient(338deg, rgba(3,0,52,1) 0%, rgba(46,38,195,1) 24%, rgba(71,62,227,1) 50%, rgba(213,59,81,1) 74%, rgba(255,164,0,1) 100%);
background: -webkit-linear-gradient(338deg, rgba(3,0,52,1) 0%, rgba(46,38,195,1) 24%, rgba(71,62,227,1) 50%, rgba(213,59,81,1) 74%, rgba(255,164,0,1) 100%);
background: linear-gradient(338deg, rgba(3,0,52,1) 0%, rgba(46,38,195,1) 24%, rgba(71,62,227,1) 50%, rgba(213,59,81,1) 74%, rgba(255,164,0,1) 100%);
filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#030034",endColorstr="#ffa400",GradientType=1);">
<img 
    src="data:image/svg+xml;base64,"
    witdh="32px" 
    height="32px" >
<img
src="https://upload.wikimedia.org/wikipedia/commons/8/8e/Nextjs-logo.svg"
witdh="32px"
style="color: white;"
height="32px">
<img
src="https://tailwindcss.com/_next/static/media/tailwindcss-mark.3c5441fc7a190fb1800d4a5c7f07ba4b1345a9c8.svg"
witdh="32px"
height="32px">
</div>
